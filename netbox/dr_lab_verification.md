...
2.  **進階災備首選**: **方案 C (異地串流溫備)**。雖然需要設定 NodePort 路由，但能換來「物理級一致性」與「一鍵秒級接管」的巨大優勢。
    *   **⚠️ 導入前評估 (規模化限制)**：由於串流溫備要求 1:1 的實例映射，若站點數量過多 (例如 >20 套)，中央叢集就必須啟動 20 個獨立的 PostgreSQL Standby StatefulSet，這會帶來極大的 CPU、記憶體與 PVC 儲存壓力。架構師需依據預算與站點規模，在「零 RPO 的串流備份」與「資源需求低的 S3 推送備份」之間取得平衡。
